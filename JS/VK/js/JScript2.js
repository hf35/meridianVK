    ...
    VK.api('audio.getRecommendations', { al: 1, id: uid, offset: 0 }, function (data) {
        ...
        if (tracks && tracks[0]) {
            for (var i = 0; i < tracks.length; i++) {
                ...
                track.innerHTML = (i + 1) + '. ' + tracks[i].artist + ' - ' + tracks[i].title;
...
App.showStartup = function () {
    ...
    h2.innerHTML = "Слушать самых популярных исполнителей на этой неделе  - ";
...
App.begin = function () {
    ...
    VK.api('storage.get', { keys: "LastFMusername, LastFMpassword, favRadio, volume, lastGroup, rand" }, function (data) {
        ...
        if (data.response[3] && data.response[3].value) {
            setTimeout(function () {
                Player.volume.setValue(data.response[3].value);
            }, 100)
        }
        Player.volume.on("change", Player.changeVolume);
        if (data.response[2] && data.response[2].value) {
            var fav = decodeURI(data.response[2].value);
            try {
        ...
        if (data.response[4] && data.response[4].value) {
            var last = decodeURI(data.response[4].value);
            try {
                App.lastGrp = JSON.parse(last);
            } catch (e) {

            }
        }
        if (data.response[5] && data.response[5].value) {
            if (data.response[5].value == 1) {
                Player.rand = true;
        ...
        var user_id = App.uid || App.parse("user_id");
...
App.radioListen = function (contin) {
    ...
    App.updatePHP = function () {
        ...
        xhr.open('GET', App.currentRadio.tags + '?' + (new Date()).getTime(), false);
            ...
            if (xhr.readyState == 4 && xhr.status == 200 && xhr.getResponseHeader('Ultra') == 'ok') {
                ...
                if ((xhr.responseText.replace(/\n/g, "") != 'error') && (xhr.responseText.replace(/\n/g, "") != "")) {
                    if (xhr.responseText != App.ultra.currentTrack && xhr.responseText.indexOf("Джингл") == -1) {
                        ...
                        var track = document.createElement('div');
                        App.ultra.i++;
                        track.innerHTML = (App.ultra.i) + '. ' + App.ultra.currentTrack;
                        Player.currentTrack = App.ultra.currentTrack;
                        div.appendChild(track);
                        LastFM.currentTrack = Player.currentTrack
                        LastFM.scronTimout = setTimeout(LastFM.scrooble, 25000);
    ...
    if (JSON) {
        VK.api('storage.set', { key: "favRadio", value: encodeURI(JSON.stringify(App.favoritesRadioList)) }, function (data) {
            if (data.response == 1) {
...
App.editTrack = function (img) {
    ...
    ok.onclick = function () {
        document.getElementById("preload").style.display = 'block';
        VK.api("audio.edit", { aid: img.data.aid, oid: img.data.owner_id, artist: artist.value, title: title.value }, function (data) {
    ...
    VK.api("audio.get", { uid: uid, /*album_id: album_id*/ }, function (data) {
        ...
        if (tracks && tracks[0]) {
            for (var i = 0; i < tracks.length; i++) {
                ...
                track.innerHTML = (i + 1) + '. ' + "<span id=aid" + i + ">" + tracks[i].artist + ' - ' + tracks[i].title + "</span>";
                track.appendChild(span);
                if (App.my) {
                    if (album_id) {
                        var all = false;
...
App.saveUser = function () {
    ...
    document.getElementById("preload").style.display = 'block';
    VK.api('storage.set', { key: "LastFMusername", value: LastFM.username }, function (data) {
        if (data.response == 1) {
            VK.api('storage.set', { key: "LastFMpassword", value: LastFM.password }, function (data) {
                if (data.response == 1) {
                    document.getElementById("preload").style.display = 'none';
...
Player.random = document.getElementById('random').onclick = function () {
    clearTimeout(Player.changeRandom);
    if (Player.rand) {
        Player.rand = false;
    ...
    Player.changeRandom = setTimeout(function () {
        var r = Player.rand ? 1 : 0;
        VK.api('storage.set', { key: "rand", value: r }, function (data) {
            if (data.response == 1) {
...
Player.seek = document.getElementById('progLoad').onclick = document.getElementById('progLive').onclick = function (e) {
    ...
    var e = e || window.event;
    var x = e.clientX;
    var offset = x - (element.offsetLeft + element.parentNode.offsetLeft + 5);
...
Player.changeVolume = function () {
    clearTimeout(Player.changeVolumeWheel.time)
    /*    var that = document.getElementById('volume');
    var e = e || window.event;
    var y = e.clientY;
    var val = y - (that.offsetTop + that.parentNode.offsetTop);
    document.getElementById('valueVolume').style.height = 52 * (val / that.offsetHeight) + 'px';
    uppodSend('player1', 'v' + (100 * (val / that.offsetHeight)))*/
    uppodSend('player1', 'v' + Player.volume.getValue());
    Player.changeVolumeWheel.time = setTimeout(function () {
        VK.api('storage.set', { key: "volume", value: Player.volume.getValue() }, function (data) {
            if (data.response == 1) {
...
Player.play = function (url, notSrcoll) {
    ...
    document.getElementById('audio').style.display = 'block';
    LastFM.artistName = Player.currentTrack.artist;
    document.getElementById('titleTrack').innerHTML = Player.currentTrack.artist + ' - ' + Player.currentTrack.title + "<span id='eq'><img src='http:/\/vk.hart-flamov.ru/JS/VK/img/eq.gif'></span>";
    ...
    var time = Math.round((new Date()).getTime() / 1000);
    var api_sig = hex_md5("api_key809dffc5f629c1d4871323f01379bc0d" + "artist" + LastFM.artistName + "methodtrack.updateNowPlaying" + "sk" + LastFM.user.sk + 'timestamp' + time + "track" + Player.currentTrack.title + "e69db535e502129eb7473f0052204099");
    var post = 'method=track.updateNowPlaying&artist=' + LastFM.artistName + '&timestamp=' + time + '&track=' + Player.currentTrack.title + '&api_key=809dffc5f629c1d4871323f01379bc0d&api_sig=' + api_sig + '&sk=' + LastFM.user.sk;
    ...
    if (Player.init) {
        ...
        var url = 'http:' + '//ws.audioscrobbler.com/2.0/';
        CROSS.XSS.post(url, post);
        document.getElementById("find").value = Player.currentTrack.artist;
        if (App.MT) {
            var scr = document.createElement('script');
            scr.src = 'http://' + 'ws.audioscrobbler.com/2.0/?method=artist.getinfo&artist=' + Player.currentTrack.artist + '&api_key=809dffc5f629c1d4871323f01379bc0d&format=json&callback=LastFM.responceArtistMT&lang=ru';
            document.body.appendChild(scr);
...
Player.findTrackByTag = function () {
    ...
    if (App.radio) {
        ...
        clearInterval(App.radioInt);
        App.ultra.currentTrack = '';
        App.s = setInterval(Player.q, 300);
    }
    App.clearVkFindStack()
...
Player.changeAlbum = function (album) {
    ...
    setTimeout(function () {
        var scr = document.createElement('script');
        scr.src = 'http://' + 'ws.audioscrobbler.com/2.0/?method=album.getInfo&artist=' + LastFM.artistName + '&album=' + album.data.name + '&api_key=809dffc5f629c1d4871323f01379bc0d&format=json&callback=LastFM.responceTracks&lang=ru';
        document.body.appendChild(scr)
    }, 100)
}
...
LastFM.responceTagTracks = function (data) {
    ...
    if (LastFM.tracks && LastFM.tracks[0]) {
        for (var i = 0; i < LastFM.tracks.length; i++) {
            ...
            span.innerHTML = time;
            span.className = 'duration';
            track.innerHTML = (i + 1) + '. ' + LastFM.tracks[i].artist.name + ' - ' + LastFM.tracks[i].name;
            track.appendChild(span)
            div.appendChild(track);
...
LastFM.responceArtist = function (data) {
    if (!data.error) {
        ...
        setTimeout(function () {
            var scr = document.createElement('script');
            scr.src = 'http://' + 'ws.audioscrobbler.com/2.0/?method=artist.getTopTracks&artist=' + LastFM.artistName + '&api_key=809dffc5f629c1d4871323f01379bc0d&limit=50&format=json&callback=LastFM.responceTopTracks&lang=ru';
            //scr.src = 'http://' + 'ws.audioscrobbler.com/2.0/?method=artist.getTopAlbums&artist=' + LastFM.artistName + '&api_key=809dffc5f629c1d4871323f01379bc0d&format=json&callback=LastFM.responceAlbums&lang=ru';
            document.body.appendChild(scr)
        }, 500)
        setTimeout(function () {
        ...
        //App.lastGrp = [];
        VK.api('storage.set', { key: "lastGroup", value: encodeURI(JSON.stringify(App.lastGrp)) }, function (data) {
            if (data.response == 1) {
...
LastFM.responceAlbums = function (data) {
    ...
    album.onclick = function () {
        var scr = document.createElement('script');
        scr.src = 'http://' + 'ws.audioscrobbler.com/2.0/?method=artist.getTopTracks&artist=' + LastFM.artistName + '&api_key=809dffc5f629c1d4871323f01379bc0d&limit=25&format=json&callback=LastFM.responceTopTracks&lang=ru';
        //scr.src = 'http://' + 'ws.audioscrobbler.com/2.0/?method=artist.getTopAlbums&artist=' + LastFM.artistName + '&api_key=809dffc5f629c1d4871323f01379bc0d&format=json&callback=LastFM.responceAlbums&lang=ru';
        document.body.appendChild(scr)
    }
    bestTracks.appendChild(album);
    ...
    album.onclick = function () {
        var scr = document.createElement('script');
        scr.src = 'http://' + 'ws.audioscrobbler.com/2.0/?method=artist.getTopTracks&artist=' + LastFM.artistName + '&api_key=809dffc5f629c1d4871323f01379bc0d&limit=100&format=json&callback=LastFM.responceTopTracks&lang=ru';
        //scr.src = 'http://' + 'ws.audioscrobbler.com/2.0/?method=artist.getTopAlbums&artist=' + LastFM.artistName + '&api_key=809dffc5f629c1d4871323f01379bc0d&format=json&callback=LastFM.responceAlbums&lang=ru';
        document.body.appendChild(scr)
    }
    bestTracks.appendChild(album);
...
LastFM.scrooble = function () {
    if (App.radio) {
        ...
        var api_sig = hex_md5("api_key809dffc5f629c1d4871323f01379bc0d" + "artist" + LastFM.artistName + "methodtrack.scrobble" + "sk" + LastFM.user.sk + 'timestamp' + time + "track" + Player.currentTrack.title + "e69db535e502129eb7473f0052204099");
        var post = 'method=track.scrobble&artist=' + LastFM.artistName + '&timestamp=' + time + '&track=' + Player.currentTrack.title + '&api_key=809dffc5f629c1d4871323f01379bc0d&api_sig=' + api_sig + '&sk=' + LastFM.user.sk;
        var url = 'http:' + '//ws.audioscrobbler.com/2.0/';
        CROSS.XSS.post(url, post);
        return 1;
    }
    if (LastFM.currentTrack == Player.currentTrack) {
        var time = Math.round((new Date()).getTime() / 1000);
...
document.getElementById('scrob').onclick = function () {
    ...
    if (LastFM.scrob) {
        field.innerHTML += '<input type="text" id="username" value="' + LastFM.userName + '" /> Имя <br /><input type="password" id="password" value="********" /> Пароль <br /> <button id="saveButton"> Сохранить </button> <button id="cancelButton"> Отмена </button> <a class="reg" href="http:\/\/www.lastfm.ru/join" > Регистрация </a>';
    } else {
        field.innerHTML += '<input type="text" id="username" /> Имя <br /><input type="password" id="password" /> Пароль <br /> <button id="saveButton"> Сохранить </button> <button id="cancelButton"> Отмена </button> <a class="reg" href="http:\/\/www.lastfm.ru/join" > Регистрация </a>';
    }
...
document.getElementById("find").onfocus = function () {
    ...
    for (var i = 0; i < 4; i++) {
        if (App.lastGrp[i]) {
            ...
            div.onclick = function () {
                document.getElementById("find").value = this.dataText
                App.suche(this.dataText);
                if (document.getElementById("tooltip")) {
                    document.getElementById("tooltip").parentNode.removeChild(document.getElementById("tooltip"));
                }
            }
