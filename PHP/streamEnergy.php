...
$titlelink = 'http://radio.mv.ru:8080/101.ru_NRJ_Rock';//'http\:\/\/94.25.53.133:80/ultra-192'; //$_REQUEST['url'];
	if ($titlelink == ""):
		echo "";
else:
	...
	//$song = substr_replace($song,"",-5); //str_replace("[ХИТрая музыка]" ,"q",$song); // str_replace("[ХИТрая музыка]","",$song);
	header('Ultra:ok');
    ...
    //Jingles//Rock Music Only!
    if (strpos($song,"Jingles") !==false ){exit;};
    if (strpos($song,"Rock Music Only!") !==false ){exit;};
    if (strpos($song,">>>") !==false ){exit;};
    if (strpos($song,"101") !==false ){exit;};
